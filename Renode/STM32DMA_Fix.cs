...
{
    public sealed class STM32DMA_Fix : IDoubleWordPeripheral, IKnownSize, IGPIOReceiver, INumberedGPIOOutput
    {
        public STM32DMA_Fix(Machine machine)
        {
        ...
        {
            public Stream(STM32DMA_Fix parent, int streamNo)
            {
                this.parent = parent;
            ...
            {
                var request = CreateRequest((int)memoryTransferType, (bufferSize - numberOfData) * (int)memoryTransferType);
                ...
                if(request.Size > 0)
                {
                    lock(parent.streamFinished)
                    {
                        ...
                        if(numberOfData == 0)
                        {
                            ...
                            if(!circular)   // Disable if not circular mode
                            {
                                Enabled = false;
                            }
                            else
                            {
                                numberOfData = bufferSize;
                            }
            ...
            private readonly STM32DMA_Fix parent;
