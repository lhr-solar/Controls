 ...
 * switches in the system, and in particular those that are present on the
 * Controls Lighting board. The driver uses GPIO to set/read pins on the MCU,
 * which in turn are connected to the Lighting Board.
 *
 */
 ...
 * @details The enum is used to index into the lookup table PINS_LOOKARR in
 * Minions.c If the enum is changed, the lookup table should be changed
 * accordingly.
 */
typedef enum {
