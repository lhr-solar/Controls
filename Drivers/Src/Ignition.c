...
ignition_state_t Get_Ignition_State(void) {
    ...
    if ((array_state + motor_state + off_state) > 1) return IGN_ERROR;
    ...
    if (!ign_has_been_reset) return IGN_OFF;
    
    if (array_state) return IGN_ARR;
    if (motor_state) return IGN_MOTOR;

    // In no states at the moment so it's in between them
