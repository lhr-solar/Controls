...
ignition_state_t Get_Ignition_State(void) {
    ...
    if (array_state == ON && motor_state == ON) return IGN_ERROR;

    // if the ignition state has been reset previously, return the value directly
    if (ign_has_been_reset) {
        if (IGN_IS_MOTOR(motor_state, array_state)) return IGN_MOTOR;
        else if (IGN_IS_ARRAY(motor_state, array_state)) return IGN_ARR;
    }
    else{
