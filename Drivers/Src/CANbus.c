...
void CarCAN_TxHandler(){
    CANbus_TxHandler(CARCAN);
}

void MotorCAN_TxHandler(){
    CANbus_TxHandler(MOTORCAN);
}

void CarCAN_RxHandler(){
    CANbus_RxHandler(CARCAN);
}

void MotorCAN_RxHandler(){
...
ErrorStatus CANbus_Init(CAN_t bus, CANId_t* idWhitelist, uint8_t idWhitelistSize)
{
    ...
    OSMutexCreate(&(CANbus_TxMutex[bus]), (bus == MOTORCAN ? "CAN TX Lock 2":"CAN TX Lock 3"), &err);
    assertOSError(err);

    OSMutexCreate(&(CANbus_RxMutex[bus]), (bus == MOTORCAN ? "CAN RX Lock 2":"CAN RX Lock 3"), &err);
    assertOSError(err);

    OSSemCreate(&(CANMail_Sem4[bus]), (bus == MOTORCAN ? "CAN Mailbox Semaphore 2":"CAN Mailbox Semaphore 3"), 3, &err); // there's 3 hardware mailboxes on the board, so 3 software mailboxes
    assertOSError(err);

    OSSemCreate(&(CANBus_ReceiveSem4[bus]), (bus == MOTORCAN ? "CAN Received Msg Queue Ctr 2":"CAN Received Msg Queue Ctr 3"), 0, &err); // create a mailbox counter to hold the messages in as they come in
    assertOSError(err);
    ...
    */
    ...
    switch(bus){
        case CARCAN:
            rxEvent = &CarCAN_RxHandler;
            txEnd = &CarCAN_TxHandler;
            break;
        case MOTORCAN:  
            rxEvent = &MotorCAN_RxHandler;
            txEnd = &MotorCAN_TxHandler;
            break;
        default:
            rxEvent = NULL;
            txEnd = NULL;
    }
    if(rxEvent == NULL || txEnd == NULL){
        return ERROR;
    }

    BSP_CAN_Init(bus, rxEvent,txEnd, (uint16_t*)idWhitelist, idWhitelistSize);
