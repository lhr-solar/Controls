...
ErrorStatus CANbus_Init(CAN_t bus, CANId_t* idWhitelist,
    ...
    OSSemCreate(
        &(CANMail_Sem4[bus]),
        (bus == CAN_1 ? "CAN Mailbox Semaphore 1" : "CAN Mailbox Semaphore 3"),
        3,
...
ErrorStatus CANbus_Send(CANDATA_t CanData, bool blocking, CAN_t bus) {
    ...
    if (blocking == CAN_BLOCKING) {
        OSSemPend(&(CANMail_Sem4[bus]), 0, OS_OPT_PEND_BLOCKING, &timestamp,
                  &err);
    } else {
    ...
    assertOSError(OS_CANDRIVER_LOC, err);
    if (retval == ERROR) {
        CANbus_TxHandler(bus);  // release the mailbox by posting back to the
                                // counter semaphore
    }
...
ErrorStatus CANbus_Read(CANDATA_t* MsgContainer, bool blocking, CAN_t bus) {
    ...
    if (blocking == CAN_BLOCKING) {
        OSSemPend(  // check if the queue actually has anything
            &(CANBus_RecieveSem4[bus]), 0, OS_OPT_PEND_BLOCKING, &timestamp,
            &err);
    } else {
    ...
    CANLUT_T entry =
        CANLUT[MsgContainer->ID];  // lookup msg information in table
    if (entry.size == 0) {
        MsgContainer = NULL;
        return ERROR;
    }  // if they passed in an invalid id, it will be zero
