...
5. In VSCode, download the following extensions (Click on the block icon on the left column):
    ...
    - ARM
    - cortex-debug
6. Add ```source /path/to/Controls/Scripts/load_env_vars.sh``` to the end of your ~/.bashrc file in order to load the necessary environment variables on shell startup. For example, mine is ```source /${HOME}/LHR/Controls/Scripts/load_env_vars.sh```
