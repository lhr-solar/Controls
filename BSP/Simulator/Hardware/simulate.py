...
### Pedals ###
accelerator = tk.Scale(
    pedal_frame, from_=1, to=0, resolution=0.001, label="accelerator"
)
...
for i, label in enumerate(Display.get_display()):
    ...
    cell.grid(
        row=i // len(display_frame_columns),
...
precharge_array_txt = tk.Label(
    master=precharge_frame, textvariable=precharge_array_status
)
...
uart_button = tk.Button(
    master=messages_frame, text="Send", command=lambda: UART.write(uart_input.get())
)
...
chargingBool.set(0)
charging_checkbox = tk.Checkbutton(
    master=charging_frame,
    text="Charging? Check if Yes",
    command=update_MotorDisable,
    variable=chargingBool,
)
