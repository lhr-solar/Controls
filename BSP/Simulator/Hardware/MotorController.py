...
def torque_control(pedalPercent):
    ...
    if CarState.mode == 1:
        CarState.CURRENT_SETPOINT = (
            pedalPercent * CarState.MAX_CURRENT
        )  # param will be a value from 0.0 to 1.0
...
class CarState:
    ...
    motor_info = [
        MOTOR_BUS_ID,
        VELOCITY_ID,
        MOTOR_PHASE_CURRENT_ID,
        MOTOR_VOLTAGE_VECTOR_ID,
    ...
    velocity_increase = (
        0.5  # how much CURRENT_VELOCITY is increased by in update_velocity()
    )

    CURRENT_SETPOINT = 0.0  # set by user via MOTOR_DRIVE commands

    MAX_CURRENT = (
        1.0  # max available current, this is a percent of the Absolute bus current
    )
