...
void BSP_CAN_Init(CAN_t bus, callback_t rxEvent, callback_t txEnd,
                  uint16_t* idWhitelist, uint8_t idWhitelistSize) {
...
void BSP_CAN1_Init(uint16_t* idWhitelist, uint8_t idWhitelistSize) {
  ...
  if (idWhitelist == NULL) {
    ...
    uint16_t* FilterStructPtr =
        (uint16_t*)&(CAN_FilterInitStruct);  // address of CAN Filter Struct
    for (uint8_t i = 0; i < idWhitelistSize; i++) {
      ...
      CAN_FilterInitStruct.CAN_FilterNumber =
          i / NUM_FILTER_REGS;  // determines filter number based on CAN ID
      *(FilterStructPtr + (i % NUM_FILTER_REGS)) = idWhitelist[i] << 5;
      validIDCounter++;

      if (i % NUM_FILTER_REGS ==
          NUM_FILTER_REGS - 1) {  // if four elements have been written to a
                                  // filter call CAN_FilterInit()
        CAN_FilterInit(CAN1, &CAN_FilterInitStruct);
      } else if (i == idWhitelistSize -
                          1) {  // we are out of elements, call CAN_FilterInit()
        for (uint8_t j = i % NUM_FILTER_REGS + 1; j <= NUM_FILTER_REGS - 1;
             j++)  // Set unfilled filter registers to 0
          *(FilterStructPtr + j) = 0x0000;

        CAN_FilterInit(CAN1, &CAN_FilterInitStruct);
      } else if (validIDCounter > 112) {  // All filter banks are to be filled
                                          // and there is no point in filtering
        for (uint8_t filter = 0; filter < 28;
             filter++) {  // Therefore, let all IDs through (no filtering)
 ...
 */
ErrorStatus BSP_CAN_Write(CAN_t bus, uint32_t id, uint8_t data[8],
                          uint8_t length) {
  ...
  uint8_t retVal =
      (CAN_Transmit(bus == CAN_1 ? CAN1 : CAN3, &gTxMessage[bus]) != 0);
