...
static void (*gRxEvent[2])(void);
static void (*gTxEnd[2])(void);

/**
 * @brief Initialize CAN1 GPIO pins
 */
static void InitCAN1Pins(void) {
...
/**
 * @brief Initialize CAN3 GPIO pins
 */
static void InitCAN3Pins(void) {
    GPIO_InitTypeDef GPIO_InitStruct;
...
void BSP_CAN_Init(CAN_t bus, void (*rxEvent)(void), void (*txEnd)(void)) {
    ...
    */
    ...
    CAN_Init(bus == CAN_1 ? CAN1 : CAN3, &CAN_InitStruct);
    ...
    CAN_FilterInit(bus == CAN_1 ? CAN1: CAN3, &CAN_FilterInitStruct);

    /* Transmit Structure preparation */
    gTxMessage[bus].ExtId = 0x5;
    gTxMessage[bus].RTR = CAN_RTR_DATA;
    gTxMessage[bus].IDE = CAN_ID_STD;
    gTxMessage[bus].DLC = 1;

    /* Receive Structure preparation */
    gRxMessage[bus].StdId = 0x00;
    gRxMessage[bus].ExtId = 0x00;
    gRxMessage[bus].IDE = CAN_ID_STD;
    gRxMessage[bus].DLC = 0;
    gRxMessage[bus].FMI = 0;

    /* Enable FIFO 0 message pending Interrupt */
    CAN_ITConfig(bus == CAN_1 ? CAN1 : CAN3, CAN_IT_FMP0, ENABLE);

    if (rxEvent != NULL) {
        // Enable Rx interrupts
        NVIC_InitStruct.NVIC_IRQChannel = bus == CAN_1 ? CAN1_RX0_IRQn : CAN3_RX0_IRQn;
        NVIC_InitStruct.NVIC_IRQChannelPreemptionPriority = 0x00; // TODO: assess both of these priority settings
    ...
    if(txEnd != NULL) {
        ...
        NVIC_InitStruct.NVIC_IRQChannelCmd = ENABLE;
        NVIC_InitStruct.NVIC_IRQChannel = bus == CAN_1 ? CAN1_TX_IRQn : CAN3_TX_IRQn;
        NVIC_Init(&NVIC_InitStruct);
    }
}
...
ErrorStatus BSP_CAN_Write(CAN_t bus, uint32_t id, uint8_t data[8], uint8_t length) {
    ...
    ErrorStatus retVal = (ErrorStatus) (CAN_Transmit(bus == CAN_1 ? CAN1 : CAN3, &gTxMessage[bus]) != 0);
...
void CAN3_RX0_IRQHandler(void) {
    ...
    // Take any pending messages into a queue
    while(CAN_MessagePending(CAN3, CAN_FIFO1)) {
        CAN_Receive(CAN3, CAN_FIFO1, &gRxMessage[CAN_3]);

        msg_t rxMsg;
        rxMsg.id = gRxMessage[CAN_3].StdId;
