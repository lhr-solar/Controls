...
void Task1(void *arg) {
  ...
  OSTaskCreate((OS_TCB *)&SendTritium_TCB, (CPU_CHAR *)"SendTritium",
               ...
               (OS_TICK)NULL, (void *)NULL, (OS_OPT)(OS_OPT_TASK_STK_CLR),
               (OS_ERR *)&err);
  // assertOSError(err);
  /**
   * ======= Forward Drive ==========
   ...
   * ======= Powered Cruise ==========
   * State Transitions:
   * Brake, Neutral, One Pedal, Forward Drive, Record Velocity, Accelerate
   * Cruise, Coasting Cruise
   */
  ...
  // goToOnePedalDrive();
  // set_cruiseSet(true);             // DOES WORK BUT NEEDS TO ENFORCE 1
  // TRANSITION set_cruiseEnable(true); set_velocityObserved(mpsToRpm(25.0));
  // stateBuffer();
  // while(get_state().name != RECORD_VELOCITY){}
