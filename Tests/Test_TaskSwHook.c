...
static char *getTCBName(OS_TCB *tcb) { return tcb ? tcb->NamePtr : "No Task"; }
...
void dumpTrace(void) {
    ...
    for (int i = 0; i < 8; i++) {
        ...
        idx = (idx + TASK_TRACE_LENGTH - 1) %
              TASK_TRACE_LENGTH;  // decrement idx, wrapping around at 0
    }
    printf("]\n\r");
}

#define expect(str)                                                 \
    printf("Previous expected: %s, previous actual: %s\n\r", (str), \
           getTCBName(PrevTasks.tasks[PrevTasks.index]))
...
int main(void) {
    ...
    OSTaskCreate((OS_TCB *)&Task1TCB, (CPU_CHAR *)"Task 1", (OS_TASK_PTR)Task1,
                 (void *)NULL, (OS_PRIO)5, (CPU_STK *)Task1Stk,
                 (CPU_STK_SIZE)DEFAULT_STACK_SIZE / 10,
                 (CPU_STK_SIZE)DEFAULT_STACK_SIZE, (OS_MSG_QTY)0, (OS_TICK)NULL,
                 (void *)NULL, (OS_OPT)(OS_OPT_TASK_STK_CLR), (OS_ERR *)&err);

    OSTaskCreate((OS_TCB *)&Task2TCB, (CPU_CHAR *)"Task 2", (OS_TASK_PTR)Task2,
                 (void *)NULL, (OS_PRIO)6, (CPU_STK *)Task2Stk,
                 (CPU_STK_SIZE)DEFAULT_STACK_SIZE / 10,
                 (CPU_STK_SIZE)DEFAULT_STACK_SIZE, (OS_MSG_QTY)0, (OS_TICK)NULL,
                 (void *)NULL, (OS_OPT)(OS_OPT_TASK_STK_CLR), (OS_ERR *)&err);
