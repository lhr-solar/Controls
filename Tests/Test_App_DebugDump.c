...
int main(void) {
  ...
  // Initialize apps
  OSTaskCreate((OS_TCB *)&Init_TCB, (CPU_CHAR *)"Init", (OS_TASK_PTR)Task_Init,
...
void Task_Init(void *p_arg) {
  ...
  OSTaskCreate((OS_TCB *)&UpdateDisplay_TCB, (CPU_CHAR *)"UpdateDisplay",
               (OS_TASK_PTR)Task_UpdateDisplay, (void *)NULL,
               (OS_PRIO)TASK_UPDATE_DISPLAY_PRIO, (CPU_STK *)UpdateDisplay_Stk,
               (CPU_STK_SIZE)WATERMARK_STACK_LIMIT,
               (CPU_STK_SIZE)TASK_UPDATE_DISPLAY_STACK_SIZE, (OS_MSG_QTY)0,
               (OS_TICK)0, (void *)NULL, (OS_OPT)(OS_OPT_TASK_STK_CLR),
               (OS_ERR *)&err);
  assertOSError(OS_MAIN_LOC, err);
  ...
  OSTaskCreate(
      (OS_TCB *)&DebugDump_TCB, (CPU_CHAR *)"DebugDump",
      (OS_TASK_PTR)Task_DebugDump, (void *)NULL, (OS_PRIO)TASK_DEBUG_DUMP_PRIO,
      (CPU_STK *)DebugDump_Stk, (CPU_STK_SIZE)WATERMARK_STACK_LIMIT,
      (CPU_STK_SIZE)TASK_DEBUG_DUMP_STACK_SIZE, (OS_MSG_QTY)0, (OS_TICK)0,
      (void *)NULL, (OS_OPT)(OS_OPT_TASK_STK_CLR), (OS_ERR *)&err);
  assertOSError(OS_MAIN_LOC, err);

  while (1) {
    Contactors_Set(MOTOR_CONTACTOR, Minions_Read(IGN_2),
