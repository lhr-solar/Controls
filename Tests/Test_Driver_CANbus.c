...
void Task1(void *p_arg) {
  ...
  dataBuf.ID =
      CHARGE_ENABLE;  // First, send a value that we want to be able to receive
  ...
  dataBuf.ID =
      ODOMETER_AMPHOURS;  // Now, send a message that we shouldn't receive
  assert(CANbus_Send(dataBuf, true, CARCAN) == SUCCESS);
  assert(CANbus_Read(&resultBuf, false, CARCAN) ==
         ERROR);  // check that no can message is read

  // Now send a bunch of messages that should be ignored, followed by those that
  // shouldn't
  for (int i = 0; i < 10; i++) {
    assert(CANbus_Send(dataBuf, true, CARCAN) == SUCCESS);
  }
