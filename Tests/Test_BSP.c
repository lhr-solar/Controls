...
int main() {
    ...
    while (1) {
        printf("%c[%d;%dH", 0x1B, 1, 1);
        printf(
            "-----PLEASE RUN THIS TEST FILE IN CONJUCTION WITH THE GUI-----\n");
        // BSP_ADC TEST
        // -----------------------------------------------------------
        printf(
            "-------------------------ADC TEST-----------------------------\n");
        ...
        // BSP_CAN TEST
        // ----------------------------------------------------------- NOTE: The
        // can test file can test both CAN1 ot CAN2 and
        ...
        printf(
            "--------------------------------------------------------------\n");
        printf(
            "CAN bus 1 sends an ID and Message, which can be seen in the "
            "GUI\n");
        ...
        printf("%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\n",
               BSP_Switches_Read(LT), BSP_Switches_Read(RT),
               BSP_Switches_Read(HDLT), BSP_Switches_Read(FWD_REV),
               BSP_Switches_Read(HZD), BSP_Switches_Read(CRS_SET),
               BSP_Switches_Read(CRS_EN), BSP_Switches_Read(REGEN),
               BSP_Switches_Read(IGN_1), BSP_Switches_Read(IGN_2));
        ...
        printf(
            "--------------------------------------------------------------\n");
        printf(
            "The UARTs are showing random values, UART 1 communicates with\n");
        ...
        int CANerr1 = rand() % 10;
        char str1[TX_SIZE];
        sprintf(str1, "%f, %d, %d, %d, %d", speed1, cruiseEn1, cruiseSet1,
                regenEn1, CANerr1);
        ...
        printf(
            "--------------------------------------------------------------\n");
        printf(
            "Contactors are constantly updated randomly, changes also visible "
            "in "
            "GUI\n");
