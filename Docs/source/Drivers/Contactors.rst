...
``ErrorStatus Contactors_Set(contactor_t contactor, bool state)`` — Sets the state of a given contactor. The change only occurs if ``contactor`` is enabled. To keep this operation thread safe, the function attempts to acquire the mutex before proceeding, releasing it at the end.
