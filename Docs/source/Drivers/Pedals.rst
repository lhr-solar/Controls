...
The Pedals driver is a thin wrapper over the ADC interface exposed by the BSP. It defines an enum that allows the driver to distinguish between the the accelerator pedal and the brake pedal. It also defines ``void Pedals_Init(void)`` (which initializes the ADC) and ``uint8_t Pedals_Read(pedal_t pedal)``, which takes a pedal and returns how far down it's pushed between 0 and 100.
