...
static const char *OS_LOC_STRING[] = {
    "OS_NONE_LOC",         "OS_ARRAY_LOC",     "OS_READ_CAN_LOC",
    "OS_READ_TRITIUM_LOC", "OS_SEND_CAN_LOC",  "OS_SEND_TRITIUM_LOC",
    "OS_UPDATE_VEL_LOC",   "OS_CONTACTOR_LOC", "OS_MINIONS_LOC",
    "OS_MAIN_LOC",         "OS_CANDRIVER_LOC", "OS_MOTOR_CONNECTION_LOC",
    "OS_DISPLAY_LOC"};

static const char *FAULT_BITMAP_STRING[] = {
    "FAULT_NONE",    "FAULT_OS",      "FAULT_UNREACH", "FAULT_TRITIUM",
    "FAULT_READBPS", "FAULT_DISPLAY", "FAULT_BPS"};
/*----------------------------------------------*/
    ...
    while (1) {
        ...
        for (pin_t pin = 0; pin < NUM_PINS; pin++) {
            ...
            printf("%s: %s\n\r", MINIONPIN_STRING[pin],
                   pinState ^ (pin == IGN_1 || pin == IGN_2) ? "on" : "off");
        }

        // Get contactor info
        for (contactor_t contactor = 0; contactor < NUM_CONTACTORS;
             contactor++) {
            bool contactorState =
                Contactors_Get(contactor) == ON ? true : false;
