...
static bool executeCommand(char *input) {
  ...
  // Iterate through all valid commands and check if the input matches (exits if
  // the action is NULL (acts as a sentinal))
...
static bool cmd_CANbus_Send(void) {
  ...
  CANDATA_t msg = {
      .ID = 0x0582,
      .idx = 0};  // this would change in the future (don't assume char as data)
