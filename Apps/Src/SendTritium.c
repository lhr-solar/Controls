 ...
 *
 * If the macro __TEST_SENDTRITIUM is defined prior to including SendTritium.h,
...
static void dumpInfo() {
	...
	printf("gear: %d\n\r", (uint8_t)gear);
	printf("currentSetpoint: ");
	print_float(currentSetpoint);
	printf("velocitySetpoint: ");
	print_float(velocitySetpoint);
	printf("velocityObserved: ");
...
static void readInputs() {
	...
	// Update buttons
	if (Minion_Read_Pin(REGEN_SW, &err) && onePedalCounter < DEBOUNCE_PERIOD) {
		onePedalCounter++;
	} else if (onePedalCounter > 0) {
		onePedalCounter--;
	}

	if (Minion_Read_Pin(CRUZ_EN, &err) &&
		cruiseEnableCounter < DEBOUNCE_PERIOD) {
		cruiseEnableCounter++;
	} else if (cruiseEnableCounter > 0) {
		cruiseEnableCounter--;
	}

	if (Minion_Read_Pin(CRUZ_ST, &err) && cruiseSetCounter < DEBOUNCE_PERIOD) {
		cruiseSetCounter++;
...
static uint8_t map(uint8_t input,
	...
	if (input <= in_min) {
		...
		uint8_t offset_in =
			input -
			in_min; // If input went from A -> B, it now goes from 0 -> B-A
...
static void ForwardDriveHandler() {
	...
	currentSetpoint = percentToFloat(map(accelPedalPercent,
