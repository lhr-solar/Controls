...
void putIOState(void) {
    ...
    // OSFlagPend(&BPS_Motor_Status_Flags, MOTOR_SAFE_TO_RUN, 0,
    //            OS_OPT_PEND_FLAG_SET_ALL | OS_OPT_PEND_NON_BLOCKING, NULL, &err);
    switch (err) {
        case OS_ERR_NONE:
        ...
        case OS_ERR_PEND_WOULD_BLOCK:
            ...
            break;
        default:
            // Should not get here because the os error is asserted in the wait
