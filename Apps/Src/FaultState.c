...
void EnterFaultState(void) {
  if (FaultBitmap & FAULT_OS) {
    ...
    tritium_error_code_t TritiumError =
        MotorController_getTritiumError();  // get error code to segregate based
                                            // on fault type
    ...
    if (TritiumError & T_HALL_SENSOR_ERR) {  // hall effect error
      ...
      if (hall_fault_cnt > RESTART_THRESHOLD) {  // try to restart the motor a
                                                 // few times and then fail out
        nonrecoverableFaultHandler();
      } else {
        hall_fault_cnt++;
        MotorController_Restart();      // re-initialize motor
        FaultBitmap &= ~FAULT_TRITIUM;  // Clearing the FAULT_TRITIUM error bit
                                        // on FaultBitmap
        return;
      }
    }
