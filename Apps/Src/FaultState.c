...
extern const pinInfo_t
    PININFO_LUT[];  // For GPIO writes. Externed from Minions Driver C file.
...
static void nonrecoverableFaultHandler() {
    ...
    BSP_GPIO_Write_Pin(PININFO_LUT[BRAKELIGHT].port,
                       PININFO_LUT[BRAKELIGHT].pinMask, true);
    ArrayMotorKill();
}
...
void EnterFaultState(void) {
    if (FaultBitmap & FAULT_OS) {
        nonrecoverableFaultHandler();
               ...
               FAULT_TRITIUM) {  // This gets tripped by the ReadTritium thread
        tritium_error_code_t TritiumError =
            MotorController_getTritiumError();  // get error code to segregate
                                                // based on fault type

        if (TritiumError &
            T_HARDWARE_OVER_CURRENT_ERR) {  // Tritium signaled too much current
            nonrecoverableFaultHandler();
        }
        ...
        if (TritiumError & T_HALL_SENSOR_ERR) {  // hall effect error
            ...
            if (hall_fault_cnt >
                RESTART_THRESHOLD) {  // try to restart the motor a
                                      // few times and then fail out
                nonrecoverableFaultHandler();
            } else {
                hall_fault_cnt++;
                MotorController_Restart();      // re-initialize motor
                FaultBitmap &= ~FAULT_TRITIUM;  // Clearing the FAULT_TRITIUM
                                                // error bit on FaultBitmap
                return;
            }
        }
        ...
        if (TritiumError &
            T_INIT_FAIL) {  // motorcontroller fails to restart or initialize
            nonrecoverableFaultHandler();
        }
         ...
         *
         * Under Voltage Lockout Error - Not really much we can do if we're not
         giving the controller enough voltage,
        ...
        return;
               ...
               FAULT_READBPS) {  // Missed BPS Charge enable message
        readBPS_ContactorHandler();

        // Reset FaultBitmap since this is a recoverable fault
        FaultBitmap &=
            ~FAULT_READBPS;  // Clear the Read BPS fault from FaultBitmap
        return;
        ...
        if (disp_fault_cnt > 3) {
            ...
            Display_Reset();
            FaultBitmap &=
                ~FAULT_DISPLAY;  // Clear the display fault bit in FaultBitmap
            return;
        }
    }
    if (fromThread) {  // no recovering if fault state was entered outside of
                       // the fault thread
        return;
    }
    while (1) {
