...
extern const PinInfo_t
	PINS_LOOKARR[]; // For GPIO writes. Externed from Minions Driver C file.
...
static void nonrecoverableFaultHandler() {
	...
	BSP_GPIO_Write_Pin(PINS_LOOKARR[BRAKELIGHT].port,
					   PINS_LOOKARR[BRAKELIGHT].pinMask,
					   true);
	ArrayMotorKill();
}
...
void EnterFaultState(void) {
	if (FaultBitmap & FAULT_OS) {
		...
		if (TritiumError & T_HALL_SENSOR_ERR) { // hall effect error
			...
			if (hall_fault_cnt >
				RESTART_THRESHOLD) { // try to restart the motor a few times and
									 // then fail out
				nonrecoverableFaultHandler();
			} else {
				hall_fault_cnt++;
	...
	if (fromThread) { // no recovering if fault state was entered outside of the
					  // fault thread
		return;
	}
	while (1) {
