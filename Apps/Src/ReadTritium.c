	...
	while (1) {
		...
		if (status == SUCCESS) {
			...
			case MOTOR_STATUS: {
				...
				Motor_FaultBitmap =
					*((uint16_t *)(&dataBuf.data[4])); // Storing error flags
			...
			case VELOCITY: {
				...
				Car_Velocity = ((Car_Velocity * 100) *
								3600); // Converting from m/s to m/h, using
									   // fixed point factor of 100
				Car_Velocity =
					((Car_Velocity / 160934) *
					 10); // Converting from m/h to mph, multiplying by 10 to
						  // make value "larger" for displaying
