...
// Saturation threshold is halfway between 0 and max saturation value (half of
// summation from one to the number of positions)
...
bool ChargeEnable_Get() { return chargeEnable; }

/**
 * @brief adds new messages by overwriting old messages in the saturation buffer
 * and then updates saturation
 * @param chargeMessage whether bps message was charge enable (1) or disable
 * (-1)
 */
...
/**
 * @brief Disables charging when canWatchTimer hits 0 (when charge enable
 * messages are missed)
 * @param p_tmr pointer to the timer that calls this function, passed by timer
 * @param p_arg pointer to the argument passed by timer
 */
...
void Task_ReadCarCAN(void *p_arg) {
	...
	while (1) {
		...
		case BPS_TRIP: {
			...
			if (dataBuf.data[0] ==
				1) { // If buffer contains 1 for a BPS trip, we should enter a
		...
		case CHARGE_ENABLE: {
			...
			if (dataBuf.data[0] ==
				...
				// If the charge message saturation is above the threshold, wait
				// for restart precharge sequence then enable charging. If we
				// are already precharging/array is on, nothing will be done
